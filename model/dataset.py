...
class TrainingDataSet(object):
    def __init__(self, sliding_dir, it_path, batch_size):
        # slding_dir is the path for virtual data and it-Path is for csv file 
        self.counter = 0
        ...
        cs = pickle.load(open(it_path,'rb'),encoding='latin1')  # this is the model for sentences   s13-d21.avi_252_452 :   # = 1604
        movie_length_info = pickle.load(open("/Users/jiazhaoli/MIMIC_DATA/SI650/exp_data/TACoS/train_clip-sentvec.pkl",'rb'),encoding = 'latin1')    # dict of movie: length   127 movie ? s36-d43'(movie name ): 26924 movie length
        # if not os.path.isfile('/scratch/qmei_fluxg/jiazhaol/train_clip_sentences_pair.pkl'):
        # print(movie_length_info[0])
        # cs list: [clipanme, sentences ]
        
        self.clip_sentence_pairs = []
        for l in cs:
            clip_name = l[0] # this is the name of video click 
            sent_vecs = l[1]# this is the embedding of sentence
            for sent_vec in sent_vecs:
                self.clip_sentence_pairs.append((clip_name, sent_vec))
            # for one clip name  s13-d21.avi_252_452 and corresponding to all sentences can be the pair. 
        movie_names_set = set()
        ...
        sliding_clips_tmp = os.listdir(self.sliding_clip_path) # for each file is 161396   s17-d48.avi_101_229.npy
        self.clip_sentence_pairs_iou = []
        for i in range(len(sliding_clips_tmp)):
            clip_name = sliding_clips_tmp[i]
        ...
        print(self.clip_sentence_pairs_iou[0])
        print(str(len(self.clip_sentence_pairs_iou))+" iou clip-sentence pairs are readed")
        # pickle.dump(self.clip_sentence_pairs_iou , open('./train_clip_sentences_pair.pkl','wb'))

        # else:
        #     with open('./train_clip_sentences_pair.pkl','rb') as f:
        #         self.clip_sentence_pairs_iou = pickle.load(f)
        #         self.num_samples_iou = len(self.clip_sentence_pairs_iou)
    '''
    ...
    def get_context_window(self, clip_name, win_length):
        ...
        return left_context_feats, right_context_feats # 3*4096 3*4096
    ...
    def next_batch_iou(self):
        ...
        while index < self.batch_size:
            ...
            if not clip_name in clip_set:
                ...
                left_context_feat=np.reshape(left_context_feat,[self.visual_feature_dim, -1])
                right_context_feat=np.reshape(right_context_feat,[self.visual_feature_dim, -1])
...
class TestingDataSet(object):
    def __init__(self, img_dir, csv_path, batch_size):
        # if not os.path.isfile('./test_clip_sentence_pairs.pkl'):
        ...
        # form the clip_sentence_pairs list 
        self.clip_sentence_pairs = []
        for l in csv:
            clip_name = l[0]
            sent_vecs = l[1]
        ...
        # pickle.dump(self.clip_sentence_pairs, open('./test_clip_sentence_pairs.pkl','wb'))
        # else:
        #     with open('./test_clip_sentence_pairs.pkl','rb') as f:
        #         self.clip_sentence_pairs_iou = pickle.load(f)
        #         self.num_samples = len(self.clip_sentence_pairs)
    ...
    def load_movie_slidingclip(self, movie_name, sample_num):
        ...
        movie_clip_featmap = []
        clip_set = set()
        # print("num of clip_sen pair"+ str(len(self.clip_sentence_pairs)))
        # print(self.clip_sentence_pairs[0])
        # print(len(self.clip_sentence_pairs[0][1]))
        # print('movie name:'+ movie_name)
        for k in range(len(self.clip_sentence_pairs)): # loop for all pairs 
            if movie_name in self.clip_sentence_pairs[k][0]:    # find the specific movie from all pairs 
                movie_clip_sentences.append((self.clip_sentence_pairs[k][0], self.clip_sentence_pairs[k][1][:self.semantic_size]))
        # print("num of movie_clip: sentences"+str(len(movie_clip_sentences)))
        # print ("num of clip: "+str(len(self.sliding_clip_names))) # for all clips 
        # print(self.sliding_clip_names[0])
        # loop for all clips in this movie, find the context of this movie 
        for k in range(len(self.sliding_clip_names)):
            if movie_name in self.sliding_clip_names[k]:
                ...
                left_context_feat,right_context_feat = self.get_context_window(self.sliding_clip_names[k]+".npy",3)
                feature_data = np.load(visual_feature_path)
                left_context_feat=np.reshape(left_context_feat,[self.visual_feature_dim,-1])
                right_context_feat=np.reshape(right_context_feat,[self.visual_feature_dim,-1])
                # print(left_context_feat.shape)
                # print(right_context_feat.shape)
                # exit()
        ...
        # return movie_clip_featmap list (name of clip and the context of this )
        # movie_clip_sentences is the lsit (name of clip and the embedding of this clip
        return movie_clip_featmap, movie_clip_sentences
