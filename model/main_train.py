...
'''
def compute_IoU_recall_top_n_forreg(move_name,clip_name,top_n, iou_thresh, sentence_image_mat, sentence_image_reg_mat, sclips, iclips):
    correct_num = 0.0
    # most_iou = 0 
    # most_start = 0
    # most_end = 0
    ...
    for k in range(sentence_image_mat.shape[0]):
        ...
        for index in picks:
            ...
            if iou>=iou_thresh:
                correct_num+=1
                break
            # f.write(clip_name+str(k)+ str(gt)+'\t'+str(gt_start)+ '\t' + str(pred_start) + '\t' + str(gt_end) + '\t'+ str(pred_end))
            # print(clip_name+str(k)+ str(gt)+'\t'+str(gt_start)+ '\t' + str(pred_start) + '\t' + str(gt_end) + '\t'+ str(pred_end))
            # exit()
    return correct_num
...
def do_eval_slidingclips(sess, vs_eval_op, model, movie_length_info, iter_step, test_result_output,context_num):
    ...
    all_retrievd = 0.0
    print(len(model.test_set.movie_names))
    for movie_name in model.test_set.movie_names:
        # if not os.path.isfile('../output/'+movie_name):
        #     with open('../output/'+movie_name,'w') as f:
        movie_length = movie_length_info[movie_name.split(".")[0]]
        print("Test movie: "+movie_name+"....loading movie data")
        # f.write("Test movie: "+movie_name+"....loading movie data")
        # f.write('\n')
        # here we got the clip-context embedding and clip_sentence embedding
        ...
        # print(sentence_image_mat.shape)
        # print(sentence_image_reg_mat.shape)
        for k in tqdm(range(len(movie_clip_sentences)-150)):
            ...
            for t in range(len(movie_clip_featmaps)):
                ...
                featmap = np.reshape(featmap, [1, featmap.shape[0], 2*context_num+1]) #batch, 4096, 7
                feed_dict = {
                model.visual_featmap_ph_test: featmap,
                model.sentence_ph_test:sent_vec
                }
                outputs = sess.run(vs_eval_op,feed_dict=feed_dict)
                outputs = list(np.reshape(outputs,[3,1]))
                sentence_image_mat[k,t] = outputs[0] # this store the similaeirty 
        ...
        iclips = [b[0] for b in movie_clip_featmaps] # list of name of vistual
        sclips = [b[0] for b in movie_clip_sentences] # list of name of sentence
        with open('../output/'+movie_name+'.txt','w') as f:
            f.write([iclips,sclips, sentence_image_reg_mat,sentence_image_reg_mat])

        # calculate Recall@m, IoU=n
        # loop for the sentence 
        for k in range(len(IoU_thresh)):
            IoU=IoU_thresh[k]
            correct_num_10 = compute_IoU_recall_top_n_forreg(movie_name,movie_clip_sentences[k][0],10, IoU, sentence_image_mat, sentence_image_reg_mat, sclips, iclips)
            correct_num_5 = compute_IoU_recall_top_n_forreg(movie_name,movie_clip_sentences[k][0],5, IoU, sentence_image_mat, sentence_image_reg_mat, sclips, iclips)
            correct_num_1 = compute_IoU_recall_top_n_forreg(movie_name,movie_clip_sentences[k][0],1, IoU, sentence_image_mat, sentence_image_reg_mat, sclips, iclips)
...
def run_training():
    ...
    context_num= 3
    pool_size = 16
    train_csv_path = '/Users/jiazhaoli/MIMIC_DATA/SI650/exp_data/TACoS/train_clip-sentvec.pkl'
    test_csv_path = 'Users/jiazhaoli/MIMIC_DATA/SI650/exp_data/TACoS/test_clip-sentvec.pkl'
    test_feature_dir='Users/jiazhaoli/MIMIC_DATA/SI650/exp_data/Interval128_256_overlap0.8_c3d_fc6/'
    train_feature_dir = 'Users/jiazhaoli/MIMIC_DATA/SI650/exp_data/Interval64_128_256_512_overlap0.8_c3d_fc6/'

    # train_csv_path = "/scratch/qmei_fluxg/jiazhaol/data/TACoS/train_clip-sentvec.pkl"
    # test_csv_path = "/scratch/qmei_fluxg/jiazhaol/data/TACoS/test_clip-sentvec.pkl"
    # test_feature_dir="/scratch/qmei_fluxg/jiazhaol/data/Interval128_256_overlap0.8_c3d_fc6/"
    # train_feature_dir = "/scratch/qmei_fluxg/jiazhaol/data/Interval64_128_256_512_overlap0.8_c3d_fc6/"

    model = GramCNN.GramCNN_Model(batch_size, pool_size, train_csv_path, test_csv_path, test_feature_dir, train_feature_dir)
    ...
    with tf.Graph().as_default():
        ...
        for step in xrange(max_steps):
            ...
            if (step+1) % 5 == 0:
                print("Start to test:-----------------\n")
                movie_length_info=pickle.load(open("/Users/jiazhaoli/MIMIC_DATA/SI650/exp_data/video_allframes_info.pkl","rb"),encoding='latin1')
